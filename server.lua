...
RegisterNetEvent('dc-casino:slots:server:enter', function(netID, ReelLocation1, ReelLocation2, ReelLocation3)
    ...
    SetTimeout(1000, function()
       ...
       -- TriggerEvent('qb-log:server:CreateLog', 'casino', 'Casino Slots', 'green', string.format("**%s** (CitizenID: %s | ID: %s) - Entered a slot | Slot NetID %s | Slot Locations %s | Reel Locations %s %s %s | Player Location %s | Slot Model %s",
        --GetPlayerName(src), Player.getName(), src, netID, SlotCoords, ReelLocation1, ReelLocation2, ReelLocation3, PlayerCoords, SlotModel))
    end)
end)
...
RegisterNetEvent('dc-casino:slots:server:spin', function(ChosenBetAmount)
    ...
    if UseCash and xPlayer.RemoveMoney('cash', SlotReferences[SlotModel].betamounts[ChosenBetAmount], 'Casino Slot Spin')
    or UseBank and xPlayer.RemoveMoney('bank', SlotReferences[SlotModel].betamounts[ChosenBetAmount], 'Casino Slot Spin')
    -- luacheck: ignore
    or UseItem and xPlayer.removeInventoryItem(ItemName, SlotReferences[SlotModel].betamounts[ChosenBetAmount]) then  else TriggerClientEvent('dc-casino:notify', src, 'Nothing left to bet with', 'error') return end
    ...
    SetTimeout(SpinTime, function()
       ...
       -- TriggerEvent('qb-log:server:CreateLog', 'casino', 'Casino Slots', 'green', string.format("**%s** (CitizenID: %s | ID: %s) - Spinned a casino slot for %s and won %s",
        --GetPlayerName(src), Player.PlayerData, src, SlotReferences[SlotModel].betamounts[ChosenBetAmount], RewardAmount))
        if RewardMultiplier == 0 then return end
        if UseCash and xPlayer.AddMoney('cash', RewardAmount, 'Casino Slot Spin')
        or UseBank and xPlayer.AddMoney('bank', RewardAmount, 'Casino Slot Spin')
        -- luacheck: ignore
        or UseItem and xPlayer.addInventoryItem(ItemName, RewardAmount) then end
    end)
end)
