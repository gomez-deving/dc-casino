...
RegisterNetEvent('dc-casino:notify')
AddEventHandler('dc-casino:notify', function(message, type)
...
function setupScaleform(scaleform)
    ...
    ButtonMessage("Change Bet!")
    PopScaleformMovieFunctionVoid()

    PushScaleformMovieFunction(scaleform, "SET_DATA_SLOT")
    PushScaleformMovieFunctionParameterInt(3)
    Button(GetControlInstructionalButton(2, 80, true))
    ButtonMessage("Max Bet!")
    PopScaleformMovieFunctionVoid()

    PushScaleformMovieFunction(scaleform, "SET_DATA_SLOT")
    PushScaleformMovieFunctionParameterInt(1)
    Button(GetControlInstructionalButton(2, 191, true)) -- The button to display
    ButtonMessage("Leave Machine!") -- the message to display next to it
    PopScaleformMovieFunctionVoid()
...
local function SlotMachineHandler()
    local InstructionButtons = setupScaleform("instructional_buttons")
    local LeverScene = 0
    local IdleScene = NetworkCreateSynchronisedScene(ClosestSlotCoord, ClosestSlotRotation, 2, 2, 0, 1.0, 0, 1.0)
    LoadAnimDict(AnimDict)
    local RandomAnimName = RandomIdle[math.random(1, #RandomIdle)]
    ...
    Citizen.CreateThread(function()
        while true do
            Citizen.Wait(0)
            DrawScaleformMovieFullscreen(InstructionButtons, 255, 255, 255, 255, 0)
        end
    end)
   -- exports['nmlrp_scripts']:DrawText('E - Bet One <br> R - Bet Max <br> ↵ - Spin <br> ← - Leave', 'left')
    CreateThread(function()
        while true do
            if not IsSpinning then
                if IsControlJustPressed(0, 202) then
                    ...
                    --exports['nmlrp_scripts']:HideText()
